...
where T1.dept_name = 'computer' group by name;
...
select distinct department.dept_name,count(*),budget as t1 from department inner join student on department.dept_name = student.dept_name group by student.dept_name;
...
delete from student where exists(select distinct student_ID from exam group by name having avg(grade) < 70);
...
select distinct name,avg(grade) from student inner join exam on student.ID = exam.student_ID where emotion_state = 'loving' group by name having avg(grade) < 75 ;
